...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		session = "94268fd01906c3b09684"
		server = "test"
		data = '{"controller":"ranking","action":"getRankAndCount","params":{"id":123,"rankingType":"ranking_Player","rankingSubtype":"population"},"session":"'+session+'"}'
		...
		for player_id in data_map :
			...
			player= None
			try :
				player = Player.objects.using('test').get(id=player_id)
		...
		Log.objects.using('test').bulk_create(log_list)
